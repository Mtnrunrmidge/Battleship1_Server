...
import OLD.jsonConverters.JsonConverter;
import OLD.jsonConverters.MessageWrapper;
import OLD.Game.GameHandler;
import java.io.*;
        ...
        if (processLogin()) {
            while (!Thread.interrupted() && socket.isConnected() && !socket.isClosed()) {
                ...
                if (Message.MessageType.CHAT.equals(message.getMessageType())) {
                    ...
                    //OLD.Game.GameHandler.handleSystemMessage((SystemMessage) message, this);
                } else if (Message.MessageType.GAME_ACTION.equals(message.getMessageType())) {
                   // OLD.Game.GameHandler.handleActionMessage((GridStatusMessage) message, this);
                }
            }
        }
    }
